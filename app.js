...
const firebaseConfig = {
  ...
  projectId: "pontoacco",
  storageBucket: "pontoacco.firebasestorage.app",
...
export async function addPunch(note='') {
  const { serverTimestamp, collection, doc, setDoc } = await import('https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js');
...
export async function listPunchesByDayAllUsers(dayISO) {
  ...
  const rows = [];
  const { getDocs: g2, collection: c2 } = await import('https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js');
